...
#define UAPPKEY "5593a55967e58ed391000c95"
...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    //读取客户端的配置信息
    std::string version =  readConfigurationFile();
    
    
    //U盟渠道统计
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
    MOBCLICKCPP_START_WITH_APPKEY_AND_CHANNEL("5593a53667e58ea18f0014d9", version.c_str());

#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS|| CC_TARGET_PLATFORM == CC_PLATFORM_MAC)
    
    MOBCLICKCPP_START_WITH_APPKEY_AND_CHANNEL("5593a55967e58ed391000c95", version.c_str());
...
std::string AppDelegate::readConfigurationFile(){
    ...
    string File_path = "res/NN_Config.json";
    //获得文件在系统的绝对路径
    string filepath = FileUtils::getInstance()->fullPathForFilename(File_path.c_str());
    if (!FileUtils::getInstance()->isFileExist(filepath)) {
        CCLOG("ERROR: Could not read res/NN_Config.json");
        return qudaohao;
    };
    //读取的内容
