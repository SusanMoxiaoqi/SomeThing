...
//******************************** 单机游戏发货模式1 游戏服务端和SDK服务端交互并通过服务端发货验证 ***************
//******************************** 处理以下三个协议即可 ***************************************************
/**
 * @brief 支付成功
...
//******************************** 单机游戏发货模式2 不经过服务端验证，直接游戏前端发货的， 处理以下协议 ***************
/**
 * @brief 单机游戏若不通过游戏服务端处理发货相关，需要处理该回调
 *          单机游戏需要在该回调中处理发放游戏道具、金币等操作
 * @param orderId XYSDK订单号码
 * @param amount 金额
 * @param 支付接口传入serverId,透传参数
 * @param extra 透传参数，可通过传入参数定位某一笔支付订单
 * @return 返回BOOL值，发货成功返回YES，发货失败返回NO， 失败的发货本地记录之后会补单
 */
- (BOOL) XYShipWithOrder:(NSString *) orderId amount:(NSString *) amount serverId:(NSString *) serverId extra:(NSString *) extra;
