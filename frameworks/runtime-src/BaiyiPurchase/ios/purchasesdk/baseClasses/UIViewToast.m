...
+ (void)makeToastActivity:(id)position {
    ...
    [[self getTop] addSubview:activityView];
...
+ (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            return CGPointMake([self getTop].bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding);
        } else if([point caseInsensitiveCompare:CSToastPositionCenter] == NSOrderedSame) {
            return CGPointMake([self getTop].bounds.size.width / 2, [self getTop].bounds.size.height / 2);
        }
    ...
    // default to bottom
    return CGPointMake([self getTop].bounds.size.width/2, ([self getTop].bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
}
...
+ (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (title != nil) {
        ...
        CGSize maxSizeTitle = CGSizeMake(([self getTop].bounds.size.width * CSToastMaxWidth) - imageWidth, [self getTop].bounds.size.height * CSToastMaxHeight);
    ...
    if (message != nil) {
        ...
        CGSize maxSizeMessage = CGSizeMake(([self getTop].bounds.size.width * CSToastMaxWidth) - imageWidth, [self getTop].bounds.size.height * CSToastMaxHeight);
...
+ (UIView*)getTop
{
    ...
    while (ctrol.presentedViewController) {
        ctrol = ctrol.presentedViewController;
    }
